...
int DrawImportPanel(AppState *app, int x, int y, int w) {
    ...
    if (app->mesh_loaded) {
        ...
        snprintf(info, sizeof(info), "Mesh: %s\nSize: %.2f x %.2f x %.2f m\nCells: %d", GetFileName(app->mesh_path),
                 size.x, size.y, size.z, app->cell_count);
...
int DrawCellPanel(AppState *app, int x, int y, int w) {
    ...
    if (app->auto_layout.use_height_constraint) {
        ...
        if (!app->auto_layout.auto_detect_height) {
            ...
            char rangeText[64];
            snprintf(rangeText, sizeof(rangeText), "Range: %.2f - %.2f m", app->auto_layout.min_height,
                     app->auto_layout.max_height);
...
int DrawSimulationPanel(AppState *app, int x, int y, int w) {
    ...
    if (app->time_sim_run) {
        ...
        snprintf(timeResults, sizeof(timeResults),
                 ...
                 "Avg Shaded: %.1f%%\n"
                 "Peak Power: %.1f W",
                 app->time_sim_results.total_energy_wh, app->time_sim_results.average_power_w,
                 app->time_sim_results.average_shaded_pct, app->time_sim_results.peak_power_w);
        ...
        if (app->string_count > 0) {
            ...
            for (int s = 0; s < app->string_count && s < 4; s++) {
                char strEnergy[48];
                snprintf(strEnergy, sizeof(strEnergy), "#%d: %.1f Wh (%.1f W avg)", app->strings[s].id,
        ...
        float theoretical_max =
                total_area * app->sim_settings.irradiance * preset->efficiency * 12.0f * avg_irradiance_factor;

        float capture_efficiency =
                (theoretical_max > 0) ? (app->time_sim_results.total_energy_wh / theoretical_max) * 100.0f : 0.0f;
    ...
    if (app->sim_run || app->time_sim_run) {
        ...
        if (app->string_count > 0 && !app->time_sim_run) {
            ...
            for (int s = 0; s < app->string_count && s < 5; s++) {
                char strPower[32];
                snprintf(strPower, sizeof(strPower), "#%d: %.1fW (%d cells)", app->strings[s].id,
                         app->strings[s].total_power, app->strings[s].cell_count);
