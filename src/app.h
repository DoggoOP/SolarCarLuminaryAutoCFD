...
// Enums
//------------------------------------------------------------------------------
typedef enum { MODE_IMPORT = 0, MODE_CELL_PLACEMENT, MODE_WIRING, MODE_SIMULATION } AppMode;

// Visualization modes for cells after simulation
typedef enum {
    VIS_MODE_STRING_COLOR = 0,   // Color by string power (green=max, red=low)
    VIS_MODE_CELL_FLUX,          // Color by cell irradiance flux (red=low, green=high)
    VIS_MODE_CELL_CURRENT,       // Color by cell current output
    VIS_MODE_SHADING,            // Show shaded vs unshaded
    VIS_MODE_BYPASS              // Highlight bypassed cells
} CellVisMode;
...
typedef struct {
    ...
    bool over_curvature_limit; // True if curvature exceeds limit
} SolarCell;
...
typedef struct {
    ...
    float edge_margin; // Minimum inset from shell edge (meters)
    bool ignore_curvature_limit; // Allow placement past curvature limit
} AutoLayoutSettings;
